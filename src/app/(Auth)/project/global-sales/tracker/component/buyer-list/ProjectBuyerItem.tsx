...
import ProjectBuyerItemManageButton
    from "@/app/(Auth)/project/global-sales/tracker/component/buyer-list/ProjectBuyerItemManageButton";
...
export default function ProjectBuyerItem({
    ...
    const handleSwitchBookmark = async () => {
        ...
        const apiRes = await callApi(`/api/admin/project/${selectedProject.id}/buyer/${buyerId}/setBookmark`, options);
        if (apiRes.result) {
    ...
    const handleSwitchPublic = async () => {
        ...
        const apiRes = await callApi(`/api/admin/project/${selectedProject.id}/buyer/${buyerId}/setPublic`, options);
        if (apiRes.result) {
            setBuyers([...buyers.map(buyer => {
                ...
                <td className={'text_left'}>
                    ...
                    <button>
                        <span className={'tracker_icon buyer_name_icon'} onClick={async () => {
                            ...
                            const apiRes = await callApi(`/api/admin/project/${selectedProject.id}/buyer/${buyer.id}/detail`, options);
                            if (apiRes.result) {
